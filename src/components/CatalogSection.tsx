...
export const CatalogSection: React.FC<CatalogSectionProps> = ({ initiallyExpanded = false }) => {
  ...
  return (
    <section className="my-6 sm:my-10 md:my-16 relative z-10 w-full" id="leituras-taro-cigano">
      <div className="max-w-6xl mx-auto px-2 sm:px-4 md:px-6">
        {/* Main Session Portal Container Card */}
        <div className="glass-card border border-gold-main/30 bg-gradient-to-b from-gold-main/[0.04] via-black/85 to-black/95 rounded-2xl sm:rounded-3xl p-4 sm:p-8 md:p-12 relative overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
          ...
          <div className="text-center max-w-3xl mx-auto relative z-10">
            <div className="inline-flex items-center gap-2 bg-gold-main/15 border border-gold-main/30 px-3 py-1 sm:px-4 sm:py-1.5 rounded-full mb-3 sm:mb-4 shadow-sm">
            ...
            <h2 className="serif text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-gold-light font-serif font-semibold mb-3 sm:mb-4 leading-tight px-1">
              Leituras de Tarot e Baralho Cigano
            </h2>

            <p className="text-white/70 font-light text-xs sm:text-sm md:text-base leading-relaxed mb-5 sm:mb-6 max-w-2xl mx-auto font-sans px-2">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 text-[11px] sm:text-xs text-white/70 font-sans font-light mb-6 sm:mb-8">
              <span className="flex items-center gap-1.5 bg-white/5 border border-white/10 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg sm:rounded-xl">
                <Layers size={13} className="text-gold-main shrink-0" />
                <span>14 Opções de Tiragens</span>
              </span>
              <span className="flex items-center gap-1.5 bg-white/5 border border-white/10 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg sm:rounded-xl">
                <Compass size={13} className="text-gold-main shrink-0" />
                <span>Respostas Objetivas & Acolhedoras</span>
              </span>
              <span className="flex items-center gap-1.5 bg-white/5 border border-white/10 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg sm:rounded-xl">
            ...
            <button
              ...
              className="button py-3.5 sm:py-4 px-6 sm:px-10 text-[11px] sm:text-xs md:text-sm font-bold uppercase tracking-[0.14em] font-sans inline-flex items-center justify-center gap-2.5 sm:gap-3 w-full sm:w-auto shadow-[0_4px_25px_rgba(201,160,74,0.25)] hover:shadow-[0_4px_30px_rgba(201,160,74,0.4)] transition-all cursor-pointer"
            >
          ...
          {isExpanded && (
            <div className="mt-8 sm:mt-12 pt-6 sm:pt-10 border-t border-white/10 relative z-10 animate-fade-in">
              {/* Category Filter Pills - Responsive horizontal scroll on mobile */}
              <div className="flex items-center justify-start md:justify-center gap-2 overflow-x-auto pb-3 mb-6 sm:mb-8 no-scrollbar -mx-2 px-2 sm:mx-0 sm:px-0">
                {categoriesList.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`shrink-0 px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-wider transition-all duration-200 font-sans cursor-pointer ${
                        isActive
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
                {filteredProdutos.map((produto) => {
                  ...
                  return (
                    <article
                      key={produto.id}
                      className={`glass-card flex flex-col justify-between group transition-all duration-300 relative overflow-hidden h-full rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 border ${
                        produto.badge
                    ...
                    >
                      <div>
                        ...
                        <div className="flex items-center justify-between mb-2.5 sm:mb-3.5 gap-2">
                          {produto.badge ? (
                            <span className="inline-flex items-center gap-1 text-[9px] uppercase tracking-widest font-bold text-gold-main bg-gold-main/15 border border-gold-main/30 px-2.5 py-0.5 rounded-full font-sans">
                              <Star size={10} className="fill-gold-main text-gold-main" />
                              {produto.badge}
                            </span>
                          ) : (
                            <span className="text-[9px] uppercase tracking-[0.18em] text-gold-main/80 font-bold font-sans">
                        ...
                        {/* Title */}
                        <h3 className="serif text-lg sm:text-xl md:text-2xl font-serif text-gold-light font-semibold mb-2 group-hover:text-gold-main transition-colors leading-snug">
                          {produto.nome}
                        </h3>
                      ...
                      {/* Card Footer */}
                      <div className="mt-auto pt-3.5 sm:pt-4 border-t border-white/10 flex items-center justify-between gap-2 sm:gap-3">
                        ...
                        <button
                          ...
                          className={`px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-bold uppercase tracking-wider transition-all duration-200 flex items-center justify-center gap-1.5 font-sans cursor-pointer shrink-0 min-h-[38px] ${
                            isAdded
