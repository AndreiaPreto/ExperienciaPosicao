...
const Membros = () => {
  ...
  const [displayName, setDisplayName] = useState('Membro');
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        if (user.displayName) {
          ...
          // Capitalize first letter of the email name
          setDisplayName(userPart.charAt(0).toUpperCase() + userPart.slice(1));
        } else {
          setDisplayName('Membro');
        }
      } else {
        // Not logged in: ProtectedRoute will handle redirect, but as safety guard:
        setDisplayName('Membro');
      }
    });
    return () => unsubscribe();
